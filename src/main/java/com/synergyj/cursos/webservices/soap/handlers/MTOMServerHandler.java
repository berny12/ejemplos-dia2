 ...
 * @version 1.0
 */
public class MTOMServerHandler implements SOAPHandler<SOAPMessageContext> {

	/**
	 * Logger para todas las instancias de la clase
	 */
	private static final Logger logger = LoggerFactory.getLogger(MTOMServerHandler.class);
	...
	public boolean handleMessage(SOAPMessageContext context) {
		...
		try {
			...
			if (outboundMessage) {
				logger.debug("Mensaje saliente lado servidor, analizando mensaje SOAP");
				ByteArrayOutputStream bos;
				bos = new ByteArrayOutputStream();
				soapMessage.writeTo(bos);
				logger.debug("tamaño aprox. del mensaje: " + bos.toByteArray().length);
			} else {
				logger.debug("Mensaje de entrada lado servidor, no se realiza accion alguna");
			}
